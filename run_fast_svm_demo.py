"""
Fast SVM demonstration with actual autoencoder data.
"""
...
def main():
    print("=" * 60)
    print("FAST SVM DEMONSTRATION WITH AUTOENCODER DATA")
    print("=" * 60)
    ...
    svm_latent = SVC(kernel='linear', C=1.0, max_iter=50, random_state=42)
    ...
    # Use even smaller subset for raw pixels
    n_raw = 500
    ...
    svm_raw = SVC(kernel='linear', C=1.0, max_iter=30, random_state=42)
    svm_raw.fit(X_train_raw, y_train_raw.ravel())
    
    # Test on subset of test data
    test_raw_indices = np.random.choice(len(X_test), 300, replace=False)
    ...
    plt.tight_layout()
    plt.savefig('report/svm_confusion_matrix_fast.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with open('fast_svm_results.pkl', 'wb') as f:
    ...
    print("\nFiles saved:")
    print("  - fast_svm_results.pkl")
    print("  - report/svm_confusion_matrix_fast.png")
